...
void PropertyManager::set_synced(int current_tick, Node *node, String property, Variant value, bool interpolate, bool deep_duplicate_value) {
  String node_path = String(node->get_path());
  String full_path = node_path + ":" + property;
  NodePath index = NodePath(property);

  Variant value_to_store = value.duplicate(deep_duplicate_value);
